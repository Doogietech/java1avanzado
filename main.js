...
//     let result = filterPokemons.length === 0 ? "Tipo de pokemon no encontrado" : filteredPokemons
...
// let Pokemon_Master = {id: 11, name: "snolax", created_date: "22/06/2022",pokemon:[] };
// console.log(Array.from(pokemons));
...
pokemons.sort(function(a,b) {a.base_damage + b.max_damage});
        let base_damage  
        let max_damage
...
function addAtributes() 
{
    ...
    for(let i=0; i<pokemons.length; i++){
        // let base_damage = Math.floor(Math.()* (2-1+1)+1) 
        // let max_damage = Math.floor(Math.random()* (5-3+1)+3)
    }
    console.log(pokemons)
